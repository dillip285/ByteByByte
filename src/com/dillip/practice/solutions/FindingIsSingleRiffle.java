...
public class FindingIsSingleRiffle {
    ...
    public static boolean isSingleRiffle(int[] half1, int[] half2, int[] shuffledDeck) {
        ...
        if((half1.length==0||half2.length==0)){
            ...
            if(shuffledDeck[shuffledDeck.length-1]==half1[half1.length-1]||shuffledDeck[shuffledDeck.length-1]==half2[half2.length-1]){
                if(shuffledDeck.length==(half1.length+half2.length)){
               return true; 
            }
            }
...
//    @Test
    ...
    public static void main(String[] args) {
        Result result = JUnitCore.runClasses(FindingIsSingleRiffle.class);
